...
sequenceDiagram
    ...
    host ->> host: sakura(host)
    host ->> host: unfree-predicate/os(host)
    host ->> host: unfree-predicate/user(host, user)
    deactivate host
    Note over host: core, default, desktop, dev<br/>host, host/resolve(host), batteries/hostname, insecure-predicate<br/>networking, printing, shell-entry, snapper<br/>unfree-predicate
    ...
    deactivate user
    Note over user: <policy:hm-user-detect>[0], default, batteries/define-user, user<br/>user/resolve(user)
```
